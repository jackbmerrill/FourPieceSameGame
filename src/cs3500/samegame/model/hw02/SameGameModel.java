...
/**
 * The operations and observations necessary for a game of
 * FunGame. These methods are shared amongst all different
 * implementations of the game, but the specific behaviors of
 * a few of them depend on the type of FunGame being implemented
 * and must be clarified in that implementation.
 *
...
public interface SameGameModel<T> {
   ...
   * the implementation of the game decides what happens and that behavior
   * must be mentioned there.</p>
   *
  ...
  /**
   * Starting from the piece at (row, col), removes all piece of the same
   * color as long as they are connected to this piece by some number of pieces
   * in cardinal directions. These connected pieces of the same color are called
   * a "matching block".
   *
   * <p>If the match results in existing pieces above missing ones,
   * the implementation of the game decides what happens then. This must
   * be mentioned in that implementation.</p>
   *
  ...
  /**
   * Returns the piece at the coordinate (row, col).
   * Returns @code{null} if no piece is at that coordinate.
   *
  ...
  /**
   * Returns the current score of the game is defined by the
  ...
  /**
   * Returns true if and only if the game is over is defined
   * by the implementation of this game.
   *
  ...
  /**
   * Starts the game with a board with the specified number of rows and columns.
   * The game allows the specified number of legal swaps until swaps fail.
   * The random variable dictates whether the board is generated
   ...
   * Missing pieces in the given board are represented with @code{null}
   * (to account for any implementation of the piece type T). The game
   * allows the specified number of legal swaps until swaps fail.</p>
   *
   ...
   * RED, GREEN, BLUE, and YELLOW. Clients can use this method
   * to create their own custom boards using the pieces understood
   * by this game.</p>
   *
